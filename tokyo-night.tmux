...
RESET="#[fg=${THEME[foreground]},bg=default,nobold,noitalics,nounderscore,nodim]"
# Highlight colors
...
#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "$RESET#[fg=${THEME[green]},bg=default] #{?#{==:#{pane_current_command},ssh},󰣀,} #[fg=${THEME[green]},bold,nodim]$window_number #W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane} #{?window_last_flag,,} "
# Unfocused
tmux set -g window-status-format "$RESET#[fg=${THEME[green]},bg=default] #{?#{==:#{pane_current_command},ssh},󰣀,}${RESET} $window_number #W#[nobold,dim]#{?window_zoomed_flag, $zoom_number, $custom_pane} #[fg=${THEME[yellow]}] "
